    ...
    <>
      <main className="w-full max-w-[430px] mx-auto flex flex-col min-h-screen bg-[#0D0D0F] border-x border-[#26262B]/60 shadow-2xl relative pb-8">
              ...
              <div className="grid grid-cols-3 gap-2.5" role="group" aria-label="Doubt resolution options">
                {['Yes', 'Partly', 'No'].map(opt => (
                  <button 
                    ...
                    type="button" 
                    className={`py-5 px-4 rounded-xl border text-lg font-semibold text-center transition-colors focus:outline-none focus:ring-1 focus:ring-[#FF3E6C] ${doubtResolved === opt ? 'bg-[#3E3E48] text-white border-[#4E4E58]' : 'border-[#2E2E36] bg-[#161619] text-[#F2F2F2] hover:bg-[#202024] hover:border-[#3D3D46]'}`}
                  >
                    {opt}
                  </button>
                ))}
        ...
        <section className="w-full flex-1 flex flex-col justify-end space-y-4 pb-8 mt-12 px-4" aria-label="Decision actions">
          ...
          <button disabled={isSharing} onClick={handleShareClick} type="button" className={`w-full py-5 px-4 bg-transparent hover:bg-[#1F1F24] active:bg-[#27272E] border-2 border-[#F2F2F2] text-[#F2F2F2] font-bold text-lg rounded-xl text-center transition-colors focus:outline-none focus:ring-1 focus:ring-[#F2F2F2] flex items-center justify-center gap-2 ${isSharing ? 'opacity-80' : ''}`}>
            {isSharing ? (
          ...
          <button onClick={() => router.push('/')} type="button" className="w-full py-4 px-4 bg-transparent hover:bg-[#161619] active:bg-[#1E1E22] text-[#F2F2F2] font-semibold text-lg rounded-xl text-center transition-colors focus:outline-none">
            Keep for later
          </button>

          {/* Button 4: "Not right for me — remove" — plain text button, mid grey */}
          <button onClick={handleDismiss} type="button" className="w-full py-4 px-4 bg-transparent hover:text-[#d4d4d8] text-[#9A9A9F] font-medium text-lg rounded-xl text-center transition-colors focus:outline-none">
